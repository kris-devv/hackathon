                    ...
                    for drone_data in drones_data:
                        ...
                        if show_prediction and len(drone.trajectory) >= 3:
                            ...
                            if prediction and 'points' in prediction:
                                ...
                                # Draw predicted trajectory as continuous line in drone color
                                ax.plot(pred_x, pred_y, 
                                       color=drone.drone_color, alpha=0.7, 
                                       linewidth=2, linestyle='-', label=f'Prediction (flight)')
                                
                                # Draw polynomial curve if coefficients available (smooth flight trajectory)
                                if 'coefficients' in prediction:
                                    from numpy import poly1d
                                    coeffs = prediction['coefficients']
                                    poly_func = poly1d(coeffs)
                                    
                                    # Generate smooth curve for flight trajectory
                                    ...
                                    ax.plot(x_range, y_range, 
                                           color=drone.drone_color, alpha=0.6, 
                        ...
                        if show_direction:
                            ...
                            ax.arrow(drone.coordinate[0], drone.coordinate[1],
                                    ...
                                    color=drone.drone_color, alpha=0.8)
                        
                        # Show fall trajectory vector (red dashed line)
                        current_velocity = drone.estimate_current_velocity()
                        if current_velocity is not None and current_velocity > 0:
                            # Pobierz wektor kierunku z ostatnich pozycji
                            dir_vec = drone.get_direction_vector()
                            # Długość wektora bazowana na prędkości (skalowanie)
                            vector_length = min(current_velocity * 10, 100)  # maksymalnie 100 pikseli
                            
                            # Oblicz punkty końcowe wektora upadku
                            end_x = drone.coordinate[0] + dir_vec[0] * vector_length
                            end_y = drone.coordinate[1] + dir_vec[1] * vector_length
                            
                            # Rysuj czerwoną przerywaną linię upadku
                            ax.plot([drone.coordinate[0], end_x], [drone.coordinate[1], end_y],
